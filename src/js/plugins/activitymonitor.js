...
class ActivityMonitor extends Plugin {
    ...
    doMonitoring() {
        if (this.enable_monitoring.innerHTML == 'Enable Monitoring') {
            this.enableMonitoring(this.pid_monitor.value,this.memory_Threshold_MB.value,this.cpu_Threshold_Percent.value,this.cpu_Threshold_Second.value,this.memory_Interval_Seconds.value,this.cpu_Interval_Seconds.value).then(response => {
            if (response.success) {
