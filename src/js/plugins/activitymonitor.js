...
class ActivityMonitor extends Plugin {
  ...
  render() {
    ...
    mainDiv.innerHTML = `
        ...
        <div class="text grid__col grid__col--8-of-8">
            <button type="button" id="memory_all" >Show for all apps</button>
        </div>
                `;
  ...
  showMemoryUsageAll() {
    var mainDiv = document.getElementById('main');
    if (this.memory_all.innerHTML == 'Show for all apps') {
      this.getAllMemoryUsage(this.pid.value).then(response => {
        if (response != null) {
          ...
          for (var i = 0; i < response.applicationMemory.length; i++) {
            ...
            mainDiv.appendChild(this.applicationName);
            mainDiv.appendChild(this.applicationNameValue);
            mainDiv.appendChild(this.pid_app);
            mainDiv.appendChild(this.pidAppValue);
            mainDiv.appendChild(this.memoryInmb);
            mainDiv.appendChild(this.memoryInmbValue);
      ...
      for (var i = 0; i < this.length; i++) {
        ...
        this.memory_all.innerHTML = 'Show for all apps';
      }
    }
  }
